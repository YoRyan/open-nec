...
function P:new(conf)
  local o = {
    ...
    _getpulsecodespeed_mps = conf.getpulsecodespeed_mps or
...
local function doenforce(self)
  while true do
    ...
    if acknowledged then
      ...
      -- the speed limit.
      self._sched:select(nil, function() return not self:issuppression() end,
...
function P:start()
  if not self._running then
    ...
    if not self._sched:isstartup() then self._sched:alert("ATC", "Cut In") end
  end
end
...
function P:issuppression() return self._getbrakesuppression() end
