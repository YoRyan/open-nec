...
function P:new(conf)
  local o = {_sched = conf.scheduler, _trigger = false}
  setmetatable(o, self)
  self.__index = self
  return o
end
...
function P:trigger() self._trigger = true end

-- Block until this event is triggered, with an optional timeout.
function P:waitfor(timeout)
  local res = self._sched:select(timeout, function() return self._trigger end)
  self._trigger = false
  return res == 1
end
