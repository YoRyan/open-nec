...
const me = new FrpEngine(() => {
    ...
    const throttle$ = frp.compose(
        ...
        mapBehavior(
            frp.liftN(
                ...
                isPenaltyBrake,
                () => Math.max(me.rv.GetControlValue("VirtualThrottle", 0) as number, 0)
            )
        )
    );
    throttle$(v => {
    ...
    const brakePipes$ = frp.compose(
        ...
        frp.map(_ => {
            return {
                application: me.rv.GetControlValue("AirBrakePipePressurePSI", 0) as number,
                suppression: me.rv.GetControlValue("MainReservoirPressurePSI", 0) as number,
            };
        })
    );
    brakePipes$(v => {
        me.rv.SetControlValue("ApplicationPipe", 0, v.application);
        me.rv.SetControlValue("SuppressionPipe", 0, v.suppression);
    });

    // Diesel exhaust
    const dieselRpm = () => me.rv.GetControlValue("RPM", 0) as number;
    ...
    const numberLight$ = frp.compose(
        me.createPlayerWithKeyUpdateStream(),
        frp.map(_ => (me.rv.GetControlValue("NumberLights", 0) as number) > 0.5),
        frp.merge(
            frp.compose(
    ...
    const engineBrakeControl$ = me.createOnCvChangeStreamFor("VirtualEngineBrakeControl", 0);
    ...
    const startupButtons$ = frp.compose(
        me.createOnCvChangeStreamFor("EngineStart", 0),
        frp.filter(v => v === 1),
        frp.merge(
            frp.compose(
                me.createOnCvChangeStreamFor("EngineStop", 0),
                frp.filter(v => v === 1),
                frp.map(_ => -1)
            )
        )
    ...
    startupButtons$(v => {
        me.rv.SetControlValue("VirtualStartup", 0, v);
        me.rv.SetControlValue("Startup", 0, v);
    });

    // Wiper control
    const wipeWipers$ = frp.compose(me.createPlayerWithKeyUpdateStream(), me.mapGetCvStream("VirtualWipers", 0));
    wipeWipers$(v => {
